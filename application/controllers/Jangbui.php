...
class Jangbui extends CI_Controller {
    ...
    public function lists()
    {
        ...
        //$base_url에서 "/" 개수를 세어 1을 더하면 된다. 예로 text1값이 없는 경우 "/jangbui/lists/page" 이므로 "/" 개수는 3이된다.
        //따라서 1을 더하면 페이지번호 위치는 4가 된다. strpos함수는 지정한 문자열이 처음 나타나는 위치 값을 알려주는 함수
        $page_segment = substr_count(substr($base_url,0,strpos($base_url,"page")) , "/")+1;

        $config["per_page"] = 2; //페이지당 표시할 line 수
        $config["total_rows"] = $this->jangbui_m->rowcount($text1); // 전체개수
        $config["uri_segment"] = $page_segment; // 페이지가 있는 segment
        $config["base_url"] = $base_url; //기본 uri "/jangbui/list/text1/값/page/페이지값"이되므로 페이지 값은 6번째에 나타난다.
        ...
        $data["text1"] = $text1;
        $data["list"] = $this->jangbui_m->getlist($text1,$start,$limit); // 해당피이지 자료읽기
    ...
    public function view()
    {
        ...
        //jangbui 클래스의 view함수에 page 정보를 적용하기 위하여 처리한다. 먼저 URI에서 page값을 읽을 수 있어야 하며, page가 없는 경우 0값으로 초기화 시킴
    ...
    {
        $config['upload_path'] = './jangbui_img';
